...
for subject_id in range(1, 15):
    ...
    # model = EEGNet(num_channels=22, seq_length=1000, num_classes=4, dropout_rate=0.5).to(device)
    model = EEGNet(num_channels=44, seq_length=1000, num_classes=4, dropout_rate=0.5).to(device)
    # model = EEGNet(num_channels=32, seq_length=768, num_classes=2, dropout_rate=0.5).to(device)
